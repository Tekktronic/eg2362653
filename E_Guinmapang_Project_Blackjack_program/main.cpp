...
class Player{ //Player class - object representing the player - has a Hand, a draw count, and an ace count
    protected:
        ...
        int HandCt;
        Card c_Card, c_Hand[5]; //creates a hand of cards with a maximum of 5 cards to a hand
        vector<Hand> Hands(4);
...
void Player::HoleCards(){
    cout << "Your cards:\n";
    cout << "[" << Hands[0].Hand[0].getRank() << " of " << Hands[0].Hand[0].getSuit() << "]|"
         << "[" << Hands[0].Hand[1].getRank() << " of " << Hands[0].Hand[1].getSuit() << "]\n";
}
...
void Player::DrawCard(Card Shoe[], int &Ct, int &t_Deck){
    ...
    if(DrawCt < 5 && t_Deck >= 0){ //Number of Cards drawn may NOT exceed 5
        c_Hand[DrawCt] = Shoe[t_Deck]; //Stores the Card drawn from the Shoe into the Hand array element c_Hand[Ct]
        Hands[DrawCt].Hand[DrawCt] = Shoe[t_Deck];
        ScoreIt(); //Score the Hand
        c_Card = c_Hand[DrawCt]; //Store the Card drawn as the Current Card;
...
void Player::ScoreIt(){
    ...
    for(x = 0; x < 5; x++){
        if(c_Hand[x].getRank() == 'A') AceCt++; //increments Player's Ace Count for every Ace drawn
        c_Card = c_Hand[x]; //sets Current Card to the Card instance in c_Hand[x]
...
void Dealer::HoleCards(){
        cout << "[* of *]|" << "[" << c_Hand[1].getRank() << " of " << c_Hand[1].getSuit() << "]" << endl;
}
//Draws a Card from the Shoe
